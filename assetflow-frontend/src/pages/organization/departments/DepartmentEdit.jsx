...
const DepartmentEdit = () => {
  ...
  useEffect(() => {
    fetchDepartmentAndMetadata();
  }, [id]);

  const fetchDepartmentAndMetadata = async () => {
    setFetchLoading(true);
    try {
      const [deptRes, empRes, targetRes] = await Promise.all([
      ...
      setParentDepartment(target.parentDepartment?._id || target.parentDepartment || '');
      setHead(target.head?._id || target.head || '');
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Modify the hierarchy or information for {name}.</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 border-t border-slate-100 pt-4">
          <div>
            ...
            >
              ...
              {departments
                .filter((d) => d._id !== id) // Prevent self-referencing hierarchy loops
                .map((dept) => (
